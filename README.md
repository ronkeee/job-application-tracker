...
This walks you through setting your display name, terminal handle, default
job role, and **role keywords** — a comma-separated list of job titles used
to find your sent application emails in Gmail (e.g. `Product Designer, UX
Designer` for a designer, or `Software Engineer, Frontend Engineer` for a
developer). Each person sharing this repo needs their own `config.json`
tuned to their role. Saves to `config.json` (not committed to git).
...
- `config.json` — your name/handle/role/roleKeywords (generated by `bun setup.js`,
  gitignored — each user has their own)
